        ...
        <table class="table text-nowrap align-middle mb-0">
          ...
          <tbody>
            <tr v-for="(product, index) in products" :key="product.productId">
              ...
              <td class="shadow-none lh-1 fs-14 fw-normal text-paragraph">
                <QuantityCounter v-model="product.purchaseQty" :min="1" :max="product.stock"
...
const props = defineProps<{ products: SelectedPurchaseProduct[] }>();

const emit = defineEmits<{
  (e: "update:products", value: SelectedPurchaseProduct[]): void;
}>();
  ...
  const updated = props.products.map((p, i) =>
    i === index
      ? {
        ...p,
        purchaseQty: newQty,
        subTotal: calculateSubtotal(p.cost, p.discount, p.tax, newQty),
      }
      : p
  );
