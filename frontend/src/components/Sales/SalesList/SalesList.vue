        ...
        <table class="table text-nowrap align-middle mb-0">
          <thead>
            <tr>
              ...
              <th scope="col" class="text-title fw-normal fs-14 pt-0">CUSTOMER
                <img class="ms-2" src="../../../assets/img/icons/up-down-aroow.svg" alt="Image" />
              </th>
              ...
              <th scope="col" class="text-title fw-normal fs-14 pt-0">PAID
                <img class="ms-2" src="../../../assets/img/icons/up-down-aroow.svg" alt="Image" />
              </th>
              <th scope="col" class="text-title fw-normal fs-14 pt-0">DUE
                <img class="ms-2" src="../../../assets/img/icons/up-down-aroow.svg" alt="Image" />
              </th>
              <th scope="col" class="text-title fw-normal fs-14 pt-0">PAYMENT STATUS
                <img class="ms-2" src="../../../assets/img/icons/up-down-aroow.svg" alt="Image" />
              </th>
          ...
          <tbody>
              ...
              <td class="shadow-none fw-normal text-black title ps-0">
                <div class="d-flex align-items-center">
                  <div class="form-check checkbox style-three">
                    <input class="form-check-input" type="checkbox" :id="'checkbox_' + sale.id" />
                    <label class="form-check-label text-optional" :for="'checkbox_' + sale.id"></label>
                  </div>
                  <span class="text-optional fs-14 ms-2">{{ sale.date }}</span>
                </div>
              </td>
              <td class="shadow-none lh-1 fs-14 fw-normal text-paragraph">{{ sale.referenceNumber }}</td>
              <td class="shadow-none lh-1 fs-14 fw-normal text-paragraph">{{ customerMap[sale.customerId] ??
                `#${sale.customerId}` }}</td>
              <td class="shadow-none lh-1 fs-14 fw-normal text-paragraph">{{ warehouseMap[sale.warehouseId] ??
                `#${sale.warehouseId}` }}</td>
              <td class="shadow-none lh-1 fs-14 fw-normal text-paragraph">
                <span class="badge badge-success fs-14 fw-normal">{{ sale.saleStatus }}</span>
              </td>
              <td class="shadow-none lh-1 fs-14 fw-normal text-paragraph">{{ currencySymbol }}{{ '00' }}</td>
              <td class="shadow-none lh-1 fs-14 fw-normal text-paragraph">{{ currencySymbol }}{{ sale.amountPaid }}</td>
              <td class="shadow-none lh-1 fs-14 fw-normal text-paragraph">{{ currencySymbol }}{{ sale.amountDue }}</td>
              <td class="shadow-none lh-1 fs-14 fw-normal text-paragraph">
                <span
                  :class="['badge fs-14 fw-normal', sale.paymentStatus === 'Paid' ? 'badge-success' : 'badge-danger']">
                  {{ sale.paymentStatus }}
                </span>
              </td>
              <td class="shadow-none lh-1 text-end pe-0">
                <div class="button-group d-flex flex-wrap align-items-center">
                  <a v-if="canView" href="javascript:void(0)" title="View" data-bs-toggle="modal"
                    data-bs-target="#detailsModal" @click="$emit('view-sale', sale)">
                    <img src="../../../assets/img/icons/eye.svg" alt="Image" />
                  </a>
                  <router-link v-if="canEdit" :to="`/edit-sales/${sale.id}`" title="Edit">
            ...
            <tr v-if="!saleStore.loading && sales.length === 0">
              <td colspan="10" class="text-center text-muted py-3">No sales found.</td>
            </tr>
          </tbody>
        </table>

        <div v-if="saleStore.loading" class="text-center py-4">
...
// Permissions
const canView = computed(() => userStore.userPermissions.includes("SALE_VIEW"));
const canEdit = computed(() => userStore.userPermissions.includes("SALE_EDIT"));
const canDelete = computed(() => userStore.userPermissions.includes("SALE_DELETE"));
</script>