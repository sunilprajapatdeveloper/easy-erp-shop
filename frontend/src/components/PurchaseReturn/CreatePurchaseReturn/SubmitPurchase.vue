...
const props = defineProps<{
  products: SelectedPurchaseReturnProduct[];
  returnTax: string;
...
const { products, returnTax, returnDiscount, shippingCost, returnStatus, note } = toRefs(props);
...
const productsTotal = computed(() =>
  products.value.reduce((sum, p) => {
    ...
    const cost = safeNumber(p.unitCost);
    const disc = safeNumber(p.discount);
    const tax = safeNumber(p.tax);
    const row = p.subTotal != null ? safeNumber(p.subTotal) : (cost - disc + tax) * qty;
    return sum + row;
  }, 0)
);
